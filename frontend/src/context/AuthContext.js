...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (name, email, password, navigate) => {
    try {
      const response = await axios.post('https://train-tickets-ih96.onrender.com/api/auth/register', {
        name,
        email,
        password,
      });
  ...
  const login = async (email, password) => {
    try {
      const response = await axios.post('https://train-tickets-ih96.onrender.com/api/auth/login', {
        email,
        password,
      });
